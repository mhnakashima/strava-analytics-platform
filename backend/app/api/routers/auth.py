...
@router.get("/callback", response_model=TokenResponse, summary="Callback OAuth — retorna JWT")
def strava_callback(code: str, db: Session = Depends(get_db)):
    try:
        token_data = strava_svc.exchange_code(code)
    except Exception as exc:
        raise HTTPException(status_code=400, detail=f"Strava OAuth error: {exc}")

    strava_athlete = token_data.get("athlete", {})
    strava_id = strava_athlete.get("id")
    if not strava_id:
        raise HTTPException(status_code=400, detail="Invalid Strava response")
