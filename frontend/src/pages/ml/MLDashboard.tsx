  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-xl font-bold text-c-ink">Machine Learning — Clusters de Treino</h1>
          <p className="text-sm text-c-ink3 mt-0.5">KMeans k=3 aplicado apenas em corridas (Run / TrailRun)</p>
        </div>
              ...
              <div key={label} className={`card p-5 space-y-3 ${isDom ? 'ring-1 ring-strava-orange/30' : ''}`}>
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-2">
                    ...
                    <span className="text-xs font-bold text-c-ink2 uppercase tracking-widest">{label}</span>
                  </div>
                  {isDom && (
      ...
      <div className="card">
        <div className="card-header">
          ...
          <span className="text-xs text-c-ink3">🟢 leve · 🟡 moderado · 🔴 intenso · Apenas corridas</span>
        </div>
        <div className="p-5">
          {isLoading ? (
        ...
        <div className="p-5 space-y-5">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-5">
            <div className="space-y-3">
              <h3 className="text-sm font-semibold text-c-ink">O algoritmo KMeans</h3>
            ...
            <div className="space-y-3">
              <h3 className="text-sm font-semibold text-c-ink">Como ler o gráfico</h3>
              <div className="space-y-2">
                ...
                <div className="space-y-2">
                  <div className="flex gap-3 items-start">
                    ...
                    <p className="text-xs text-c-ink2"><span className="text-emerald-400 font-medium">Leve</span> — rápido e curto: intervalos de velocidade, aquecimentos</p>
                  </div>
                  <div className="flex gap-3 items-start">
                    <span className="w-2.5 h-2.5 rounded-full bg-amber-500 mt-1 shrink-0" />
                    <p className="text-xs text-c-ink2"><span className="text-amber-400 font-medium">Moderado</span> — ritmo sustentado: corridas de base, tempo runs</p>
                  </div>
                  <div className="flex gap-3 items-start">
                    <span className="w-2.5 h-2.5 rounded-full bg-red-500 mt-1 shrink-0" />
                    <p className="text-xs text-c-ink2"><span className="text-red-400 font-medium">Intenso</span> — longo e pesado: corridas longas, trails com elevação</p>
                  </div>
                </div>
              </div>
            </div>
          ...
          {profile && (
            <div className="bg-strava-orange/5 border border-strava-orange/20 rounded-xl p-4 space-y-2">
              <div className="flex items-center gap-2">
                ...
                <h3 className="text-sm font-semibold text-c-ink">Análise do seu perfil de treino</h3>
              </div>
              <p className="text-sm text-c-ink2 leading-relaxed">
                O princípio <span className="text-c-ink font-medium">80/20</span> de polarização diz que ~80% dos treinos devem ser fáceis e ~20% intensos.
              ...
              <div className="flex gap-4 text-xs text-c-ink3">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
            {(['leve', 'moderado', 'intenso'] as const).map((label) => {
              const meta = CLUSTER_META[label];
              return (
                <div key={label} className="bg-c-raised/70 rounded-xl p-4 space-y-2">
                  <div className="flex items-center gap-2">
                    <div className="w-2.5 h-2.5 rounded-full" style={{ background: clusterColor(label) }} />
                    <span className="text-xs font-bold text-c-ink uppercase tracking-wide">{label}</span>
                  </div>
